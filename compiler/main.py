...
reserved_word=['print', 'while', 'if', 'else', 'def', 'use', 'item', 'buy']
...
class Buy(Node):
    ...
    def evaluate(self, st):
        item = st.get_key_by_value(self.value) 
        if item is None:
            raise Exception('Marca Errada')
class Print(Node):
    def __init__(self, value, children):
        self.value = value
        self.children = children
...
class Parser:
    ...
    def parseStatement(self):
        if self.tokenizer.next.type == 'NEWLINE':
            self.tokenizer.selectNext()
            return NoOp()
        elif self.tokenizer.next.type == 'RESERVED' and self.tokenizer.next.value == 'buy':
            self.tokenizer.selectNext()
            if self.tokenizer.next.type == 'NAME':
